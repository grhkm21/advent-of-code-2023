...
def parse_map:
    split(" ")
    | map(tonumber)
    | if length == 3 then { "dest": .[0], "src": .[1], "len": .[2] } else .[] end;

def parse_data:
    [inputs]
    | join("\n")
    | split("\n\n")
    | map(split(":").[1] | strip | split("\n") | map(parse_map));
...
def process_maps1($data; $maps):
    $maps
    | map(select(.src <= $data and $data < .src + .len) | .dest + $data - .src)
    | if (. == []) then $data else .[0] end;
